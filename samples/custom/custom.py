...
classess = ["bg", "bonet","windshield", "door", "fender", "boot"]
...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        self.add_class("object", 1, "bonet")
        self.add_class("object", 2, "windshield")
        self.add_class("object", 3, "door")
        self.add_class("object", 4, "fender")
        self.add_class("object", 5, "boot")
        # self.add_class("object", 3, "xyz")
        name_dict = {"bonet": 1,"windshield": 2, "door":3, "fender": 4, "boot":5}
...
def train(model):
    ...
    dataset_val.load_custom(args.dataset, "train")
    dataset_val.prepare()
...
def detect_(model, image_path=None):
    if image_path:
        # Run model detection and generate the color splash effect
        print("Running on {}".format(args.image))
        # Read image
...
if __name__ == '__main__':
    ...
    NEW_WEIGHTS_PATH = "D:\\Tests\\maskRCNN\\Mask_RCNN\\logs\\object20210526T1118"
    NEW_WEIGHTS_NAME  = "mask_rcnn_object_0012.h5"

    TEST_SAMPLES_PATH = "D:\\Tests\\maskRCNN\\Mask_RCNN\\samples\\custom\\test"
    # Select weights file to load
    if args.weights.lower() == "new":	
        print("weight path entered")	
        print(NEW_WEIGHTS_NAME)	
        weights_path = os.path.join(NEW_WEIGHTS_PATH, NEW_WEIGHTS_NAME)
    ...
    if args.command == "train":
        ...
        image_path = os.path.join(TEST_SAMPLES_PATH, args.image)
        detect_(model, image_path=image_path)
    else:
